            ...
            <section class="invoice">
              ...
              <div class="row">
                <div class="col-xs-12">
                  <h2 class="page-header">
                    <i class="fa fa-globe"></i><?php echo $row_account['account_authorized_name_surname']  ?>
              ...
              <div class="row invoice-info">
                <div class="col-sm-4 invoice-col">
                  ...
                  <address>
                    <strong></strong><br>
                   Ad-Soyad: <?php echo $row_account['account_authorized_name_surname'] ?><br>
                   IBAN: <?php echo $row_account['account_iban'] ?><br>
                    E-posta: <?php echo $row_account['account_email'] ?><br>
                    Telefona: <?php echo $row_account['account_phone'] ?><br>
                    Hesap Tipi: <?php echo $row_account['type'] ?><br>
                    Para Birimi: <?php echo $row_account['birim'] ?><br>
                  </address>
                </div>
                <!-- /.col -->
                <div class="col-sm-4 invoice-col">
                ...
                <table class="table">
                  <tbody><tr>
                    ...
                    <td>
                      ...
                      ?><?php $sorgu=$db->qSQL("SELECT * FROM account WHERE account_id=:d");
                      $sorgu->execute(array(":d"=>$_GET['account_id']));
                      $row = $sorgu->fetch(PDO::FETCH_ASSOC);
                       $hesapturu = $row["birim"];
                       if($hesapturu=="TL"){
                         echo "₺";
                       }else if($hesapturu=="DOLAR"){
                         echo "$";
                       }else{ echo " ";}
                      ?>
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      $bakiye=($sales_total-$revenue);
                      $guncelle = $db -> qSQL("UPDATE account SET bakiye=:bakiye WHERE account_id=:id");
                      $guncelle -> execute(array(":id" => $_GET["account_id"], ":bakiye" => $bakiye));
                      if ($guncelle){
                        echo $bakiye;
                      }    ?><?php $sorgu=$db->qSQL("SELECT * FROM account WHERE account_id=:d");
                           $sorgu->execute(array(":d"=>$_GET['account_id']));
                  ...
                  <tr>
                    <th>Kar:</th>
                    <td>
                      <?php echo number_format($sales_total-$expense); ?> <?php $sorgu=$db->qSQL("SELECT * FROM account WHERE account_id=:d");
                           $sorgu->execute(array(":d"=>$_GET['account_id']));
                           $row = $sorgu->fetch(PDO::FETCH_ASSOC);
                            $hesapturu = $row["birim"];
                            if($hesapturu=="TL"){
